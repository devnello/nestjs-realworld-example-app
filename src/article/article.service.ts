...
export class ArticleService {
    ...
    async findAll2(): Promise<{}> {
        ...
        const articles = await getRepository(ArticleEntity).query('SELECT * FROM article');
        console.log(articles);

        const articlesCount = 2;

        // return rowData; // Buena
        return {articles: articles, articlesCount: articlesCount};
    }

    async findAll(query): Promise<ArticlesRO> {

        const rowData = await getRepository(ArticleEntity).query('SELECT * FROM article');
        console.log(rowData);
